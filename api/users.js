        ...
        try {
          ...
          if (dbRes && Array.isArray(dbRes.rows) && dbRes.rows.length > 0) {
            const mapped = dbRes.rows.map(r => ({
              ...
              assignedProjects: r.assigned_projects || (r.assigned_project_id ? [r.assigned_project_id] : (r.workspace_access || ['creativoffice'])),
              assignedBoardNames: r.assigned_board_names || [r.assigned_board_name || 'CreativOffice'],
              workspaceAccess: r.workspace_access || (r.assigned_projects ? r.assigned_projects : [r.assigned_project_id || 'creativoffice']),
